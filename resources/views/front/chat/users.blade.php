<div class="md:w-3/12" x-data="{ open: false }">
    <div class="relative">
        ...
        <div x-show="open" @click.away="open = false" class="origin-top-right absolute right-0 mt-2 w-full rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 max-h-60 overflow-y-auto">
            <div class="py-1">
                @foreach($users as $user)
                <a href="{{ route('chat.show', $user->id) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">{{ $user->first_name }} {{ $user->last_name }}</a>
                @endforeach
            </div>
        </div>
    </div>
